...
{
    internal static class EnemyCompatibilityAnimationAuthoring
    {
        private const string SelfPath = "Assets/_LizzoPV/Editor/EnemyCompatibilityAnimationAuthoring.cs";
        private const string SourceRoot = "Assets/_LizzoPV/Animations/Units";
        private const string DestinationRoot = "Assets/_LizzoPV/Gameplay/Enemies/Animations/Compatibility";
        private const string EnemyPrefabRoot = "Assets/_LizzoPV/Gameplay/Enemies/Prefabs/Units";
        ...
        public static void Apply()
        {
            ...
            try
            {
                ...
                foreach (string enemyName in EnemyNames)
                {
                    ...
                    {
                        string assetName = enemyName + suffix;
                        ...
                        if (string.IsNullOrEmpty(sourceGuid))
                            throw new InvalidOperationException($"Enemy compatibility animation is missing: {sourcePath}");
                ...
                foreach (string enemyName in EnemyNames)
                {
                    ValidateControllerAndClips(enemyName);
                    ValidateEnemyPrefabController(enemyName);
                }

                succeeded = true;
                Debug.Log("[Enemy Compatibility Animations] PASS: 25 assets moved with GUIDs, clips, and 5 Enemy Prefab bindings preserved.");
            }
            finally
            {
                if (succeeded)
        ...
        private static void ValidateControllerAndClips(string enemyName)
        {
            string controllerPath = DestinationRoot + "/" + enemyName + ".controller";
            RuntimeAnimatorController controller = AssetDatabase.LoadAssetAtPath<RuntimeAnimatorController>(controllerPath);
            if (controller == null)
                throw new InvalidOperationException($"Enemy controller is not importable: {controllerPath}");
            ...
            if (clips == null || clips.Length != expectedNames.Count)
                throw new InvalidOperationException($"{enemyName} controller clip count changed: expected={expectedNames.Count}, actual={clips?.Length ?? 0}");

            foreach (AnimationClip clip in clips)
            {
                if (clip == null || expectedNames.Remove(clip.name) == false)
                    throw new InvalidOperationException($"{enemyName} controller has an unexpected clip: {clip?.name ?? "<null>"}");
            }

            if (expectedNames.Count != 0)
                throw new InvalidOperationException($"{enemyName} controller is missing one or more expected clips.");
        }

        private static void ValidateEnemyPrefabController(string enemyName)
        {
            RuntimeAnimatorController expectedController = AssetDatabase.LoadAssetAtPath<RuntimeAnimatorController>(
                DestinationRoot + "/" + enemyName + ".controller");
            string prefabPath = EnemyPrefabRoot + "/" + enemyName + ".prefab";
            GameObject enemyPrefab = AssetDatabase.LoadAssetAtPath<GameObject>(prefabPath);
            if (enemyPrefab == null)
                throw new InvalidOperationException($"Enemy Prefab is not importable: {prefabPath}");

            Animator[] animators = enemyPrefab.GetComponentsInChildren<Animator>(true);
            ...
            throw new InvalidOperationException($"{enemyName} Prefab no longer resolves its compatibility Animator Controller.");
        }
