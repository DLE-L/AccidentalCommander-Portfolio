...
namespace Lizzo.PV.EditorTests
{
    public sealed class CompanionRecordingProductionCutoverTests
    {
        ...
        public void RecordingDefinitionCatalog_UsesExactlyFiveCanonicalLineagesAndDataDrivenSwordRoles()
        {
            ...
            Assert.That(sword.BaseActionSet.Steps[0].ActionDurationSeconds, Is.EqualTo(0.30f));
            Assert.That(sword.BaseActionSet.Steps[0].ExcursionStandOffDistance, Is.EqualTo(0.9f));
            Assert.That(sword.BaseActionSet.Steps[0].ExcursionLateralOffset, Is.EqualTo(0.28f));
            Assert.That(sword.PromotedActionSet.Steps.Count, Is.EqualTo(2));
            Assert.That(sword.PromotedActionSet.Steps[0].Motion, Is.EqualTo(CombatMotion.Stationary));
            Assert.That(sword.PromotedActionSet.Steps[1].Motion, Is.EqualTo(CombatMotion.Stationary));
        ...
        [Test]
        public void FixedCardPool_WithRecordingInputRoutesCanonicalCardOnlyToNewRuntime()
        {
            ...
            try
            {
                ...
                RunServices run = new RunServices(app, new Lizzo.PV.Flow.RunState(), registry, pool, factory);
                FixedCardPool.Configure(
                    run.Registry,
                    run.Party,
                    companionCardInput: input,
                    companionRosterView: input);
                ...
                try
                {
                    ...
                    Assert.That(input.SubmittedIds, Is.EqualTo(new[] { "shield_guard" }));
                    Assert.That(FixedCardPool.TryApplyCard(new CardData(
                        CardKind.AddShieldSoldier,
                        "legacy",
                        "legacy",
                        CardHighlight.None)), Is.False);
                    Assert.That(input.SubmittedIds.Count, Is.EqualTo(1));
        ...
        private sealed class RecordingInput : ICompanionCardInput, ICanonicalCompanionRosterView
        {
            internal List<string> SubmittedIds { get; } = new List<string>();
            ...
            {
                SubmittedIds.Add(canonicalCompanionId);
                return new CompanionRosterCommandResult(true, CompanionRosterRejection.None, "squad-0", 0);
            }
        }

        private sealed class NullPrefabFactory : IPrefabFactory
        {
