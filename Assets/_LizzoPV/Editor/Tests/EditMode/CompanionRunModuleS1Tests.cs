...
namespace Lizzo.PV.EditorTests
{
    public sealed class CompanionRunModuleS1Tests
    {
        ...
        public void RecruitToDirectResolution_TraversesFacadeDeterministically()
        {
            ...
            Assert.That(events[1].Kind, Is.EqualTo(CompanionRunEventKind.EffectCommitted));
            Assert.That(events[1].PresentationCue.Value.PresentationId, Is.EqualTo("slash-basic"));
            Assert.That(events[2].Order, Is.EqualTo(3L));
            Assert.That(events[2].Kind, Is.EqualTo(CompanionRunEventKind.EffectResolved));
            Assert.That(events[2].Resolution.Value.Applied, Is.True);
            Assert.That(events[2].Resolution.Value.AppliedMagnitude, Is.EqualTo(12.0f));
            Assert.That(events[2].Resolution.Value.AffectedTargetCount, Is.EqualTo(1));
            Assert.That(events[2].PresentationCue.Value.PresentationId, Is.EqualTo("slash-basic"));
