    ...
    public sealed class OwnerBoundSupportPresenterTests
    {
        ...
        public void Observe_CapsOneVisual_ForwardsCueAndReleasesIdempotently()
        {
            ...
            try
            {
                ...
                Assert.AreEqual(1, factory.CreateCount); Assert.IsTrue(presenter.IsPresenting); Assert.AreEqual(Vector3.up, factory.Visual.Position); Assert.AreEqual(Vector3.left, factory.Visual.Facing); Assert.AreEqual("Attack", factory.Visual.Motion);
                presenter.Observe(WolfOwnedProxyPhase.Inactive, Vector3.zero, Vector3.zero);
                presenter.Release(); Assert.AreEqual(1, factory.ReleaseCount); Assert.IsFalse(presenter.IsPresenting);
            }
        ...
        {
            public int CreateCount; public int ReleaseCount; public bool Fail; public readonly FakeVisual Visual = new FakeVisual();
            public bool TryCreate(OwnerBoundSupportPresentationData data, out IOwnerBoundSupportVisual visual) { CreateCount++; visual = Fail ? null : Visual; return Fail == false; }
            public void Release(IOwnerBoundSupportVisual visual) { ReleaseCount++; }
        }
        private sealed class FakeVisual : IOwnerBoundSupportVisual
        {
            public Vector3 Position; public Vector3 Facing; public string Motion;
