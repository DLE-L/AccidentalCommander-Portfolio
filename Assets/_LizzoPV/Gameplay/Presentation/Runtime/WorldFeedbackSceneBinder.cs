...
namespace Lizzo.PV.Gameplay.PresentationRuntime
{
    ...
    public sealed class WorldFeedbackSceneBinder : MonoBehaviour
    {
        ...
        private void Awake()
        {
            if (_runBootstrap == null || _profiles == null)
            {
                Debug.LogError("[WorldFeedbackSceneBinder] Authored RunBootstrap and Profile Set are required.", this);
                enabled = false;
                return;
            }
            ...
            if (_screenOverlay != null)
            {
                ...
                if (_screenOverlay != null) _screenOverlay.gameObject.SetActive(false);
            }
        }
        ...
        {
            CommanderActor commander = _runBootstrap.Services.Registry.Player;
            if (commander == null)
                return;
            ...
            if (_screenOverlay != null) _screenOverlay.gameObject.SetActive(true);
        ...
        {
            foreach (EnemyActor enemy in _runBootstrap.Services.Registry.Enemies)
            {
        ...
        private void PresentStatusReaction(StatusFeedbackProfileSO profile, StatusFeedbackPresentation presentation)
        {
            ...
            for (int index = 0; index < reactions.Count; index++)
            {
                ...
                if (reaction.ReactionKind != presentation.ReactionKind)
                    continue;
                SpawnVfx(reaction.ReactionVfxId, presentation.Position, presentation.VisualScale);
        ...
        private void SpawnVfx(VfxAssetId id, Vector3 position, float scale = 1f)
        {
            try { SpawnVfxCore(id, position, scale); }
            catch (Exception exception) { Debug.LogException(exception, this); }
        }

        private void SpawnVfxCore(VfxAssetId id, Vector3 position, float scale)
        {
        ...
        private void PlayAudio(AudioAssetId id)
        {
            ...
            try { PlayAudioCore(id); }
            catch (Exception exception) { Debug.LogException(exception, this); }
        }

        private void PlayAudioCore(AudioAssetId id)
        {
            ...
            _audioSource.PlayOneShot(clip, _profiles.WorldSfxVolume);
        }

        private void ApplyOverlaySprite(SpriteAssetId id)
        {
            if (_screenOverlay != null && TrySprite(id, out Sprite sprite))
        ...
        {
            if (_screenMotion == null || id.IsNone || _catalogs == null || !_catalogs.MotionCatalog.TryGet(id, out AnimationClip clip))
                return;
