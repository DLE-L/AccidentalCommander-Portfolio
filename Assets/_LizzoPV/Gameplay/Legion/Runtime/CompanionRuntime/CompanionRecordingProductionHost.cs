...
namespace Lizzo.PV.Legion.RunCore
{
    ...
    internal static class CompanionMemberVisualVariant
    {
        public static float ResolveIntensity(int memberOrder)
        {
    ...
    public sealed class CompanionRecordingDefinitionCatalog : ICompanionDefinitionCatalog
    {
        ...
        private static CompanionDefinition CreateDefinition(IDataProvider data, string companionId)
        {
            ...
            float baseActionDuration = baseMotion == CombatMotion.Excursion ? SwordExcursionActionDuration : 0.0f;
            float baseExcursionSpeed = baseMotion == CombatMotion.Excursion ? SwordExcursionSpeed : 0.0f;
            float baseExcursionStandOff = baseMotion == CombatMotion.Excursion ? SwordExcursionStandOff : 0.0f;
            float baseExcursionLateral = baseMotion == CombatMotion.Excursion ? SwordExcursionLateral : 0.0f;
            ...
            {
                promotedSteps.Add(CreateSecondaryHealStep(secondaryEffect, promotion.EffectMultiplier));
            }
    ...
    internal sealed class CompanionRecordingCombatWorld : ICompanionCombatWorld, IRangedCompanionTargetWorld
    {
        ...
        private static void PresentRecordingVideoEffect(
            string effectId,
