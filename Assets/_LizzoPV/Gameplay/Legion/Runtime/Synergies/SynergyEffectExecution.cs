...
namespace Lizzo.PV.Gameplay.Run
{
    ...
    internal sealed class SynergyEffectExecutor
    {
        ...
        private readonly List<EnemyActor> _targets = new List<EnemyActor>(32);
        ...
        internal void Execute(string sourceId, in SynergyEffectCommand command)
        {
            ...
            {
                EnemyActor target = _targets[index];
                if (damage > 0)
                {
        ...
        private static void HealCommander(CommanderActor commander, float magnitude)
        {
            if (commander == null || magnitude <= 0.0f)
                return;

            int amount = Mathf.Max(1, Mathf.RoundToInt(magnitude));
            commander.Heal(amount);
