...
public partial class EnemyActor
{
	public bool TryCancelChargeAndApplyStun(float stunDuration, out ChargeCancellationResult result)
	{
		...
		IChargeCancelable chargeCancelable = _chargeAttack;
	...
	void EnsureRuntimeComponents()
	{
		...
		if (_chargeAttack == null)
			_chargeAttack = GetComponent<EnemyChargeController>();
		if (_hungryGiant == null)
			_hungryGiant = GetComponent<EnemyBossController>();
		if (_bossVulnerability == null)
			_bossVulnerability = GetComponent<BossVulnerabilityWindow>();
	...
	static Collider2D ResolveCombatCollider(Transform owner)
	{
		...
		CommanderActor player = owner.GetComponent<CommanderActor>();
		if (player != null)
			return player.CombatCollider;

		EnemyActor monster = owner.GetComponent<EnemyActor>();
		if (monster != null)
